...
export default class TokenSale extends Component {
    ...
    async __callBuyTokens() {
        const hideLoad = message.loading(`Purchased ${this.state.input_token} ${this.state.token_symbol}...`, 0)
        if (this.state.input_bnb != 0 && this.state.input_token != 0) {
            try {
                ...
                this.setState({
                    ...
                    hideLoad()
                    message.success({ content: `Tokens purchased successfully!`, key: this.state.account, duration: 2 })
                    this.load()
                })
            } catch (err) {
                hideLoad()
            ...
            hideLoad()
            message.error({ content: `You need to specify a purchase amount`, key: this.state.account, duration: 2 })
        }
    }
            ...
            return (<>
                <Card>
                    <Row>
                            ...
                            <Steps current={this.state.phase.phase - 1} percent={this.getPercentage()} direction="vertical">
                                {this.state.phases.map((phase, i) => {
                                    return <Step key={this.state.phase.phase - 1} title={`FASE ${phase.phase}`}
                                        description={`Remaining tokens: ${phase.total}, price = inicial x${phase.phase}`}
                        ...
                        <Col span={12}>
                            <Row>
                                {this.state.phase.phase == 10 && this.state.phase.total <= 1 ? <Button disabled type="primary" shape="round">Not available</Button>
                                    : <Button type="primary" shape="round" onClick={() => this.buyTokens()}>BUY TOKENS</Button>}
                            </Row>
                        </Col>
                    </Row>
                </Card>
                <Modal
                    title="Buy CristoDapp Tokens"
