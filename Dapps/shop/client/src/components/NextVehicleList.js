...
class NextGamesList extends Component {
        ...
        if (isConnected()) {
            ...
            this.setState({
                ...
                if (this.state.section)
                    this.load()
                else this.props.history.push('/cars')
            })
    ...
    copyAddr() {
        ...
        message.success({ content: '¡Contact copied!', key: this.state.account, duration: 3 })
    }
        ...
        return (
            <div className="popover">
                ...
                <Tag
                    ...
                    {this.state.itemDetailsContact ? this.state.itemDetailsContact : 'Without contact'}
                </Tag>
    ...
    getOpenPurchaseButton(item) {
        return (
            _isPurchased(item.reserved_by) ?
                <Button danger disabled shape="round">Purchased</Button> :
    ...
    async approve() {
        const hideLoad = message.loading(`Aprove ${_weiToBNB(this.state.itemToBuy.price)} ${this.state.token_symbol}...`, 0)
        try {
            ...
            this.setState({
                ...
                hideLoad()
                message.success({ content: `${_weiToBNB(this.state.itemToBuy.price)} ${this.state.token_symbol} Approved successfully!`, key: this.state.account, duration: 2 })
            })
        } catch (err) {
            hideLoad()
    ...
    async buyItem() {
        const hideLoad = message.loading(`Purchased ${this.state.itemToBuy.name}...`, 0)
        ...
        try {
            ...
            this.setState({
                ...
                hideLoad()
                message.success({ content: `${this.state.itemToBuy.name} Purchase successful!`, key: this.state.account, duration: 2 })
                this.load()
            })
        } catch (err) {
            hideLoad()
    ...
    getApproveButton() {
        return (this.state.itemToBuy.price > this.state.allowance ?
            <Button key="approve" shape="round" type="primary" onClick={() => { this.approve() }}>Buy</Button> :
            <Button disabled key="approve" shape="round" type="primary">Approve</Button>)
    }

    getPurchaseButton() {
        return (this.state.itemToBuy.price <= this.state.allowance ?
            <Button key="purchase" shape="round" type="primary" onClick={() => { this.buyItem() }}>Buy</Button> :
            <Button disabled key="purchase" shape="round" type="primary">Buy</Button>
        )
    }
